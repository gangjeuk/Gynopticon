...
def exp_from_arguments():
    ...
    try:
        arg.frames, arg.cheater, arg.times =\
            pd.read_pickle(open('trained_models/_frames', 'rb'))
...
def anom_score(m, e, key, use_all=True, return_all=False):
    ...
    Returns
    ----------
    y_true: true label -> is cheater
    mx_score: max score -> max score of player
    """
...
def anom_times_all(m, e, key, use_all=True):
