...
def inference(dataset, model, batch_size):
    ...
    return (
        np.concatenate(ts), # time stamp
        np.concatenate(dist), # answer - guess
        np.concatenate(att), # 
    )
...
# should be process_diff? MAYBE?
def process_dist(ts, dist, method='max'):
    new_ts, new_dist, new_score = [], [], []

    score = np.linalg.norm(dist, ord=1.0, axis=1)/dist.shape[1]
    # consecutive groups
    # 가까운 거리에 있는 애들을 기준으로 그룹화 한다
    # FROM: [1, 10, 11, 12, 20, 30, 31, 32, 33, 40]
    # TO: [1], [10, 11, 12], [20], [30, 31, 32, 33], [40]
    # lamda 를 살펴보면 timestamp 기준으로 그룹화를 하는 것을 알 수 있다
